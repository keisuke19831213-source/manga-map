...
import { historyText, workTitle } from "@/lib/content-en";
...
const ERAS: Era[] = [
  {
    era: "12C—1860s",
    title: "前史 — 絵で語る伝統",
    body: "平安末期の『鳥獣人物戯画』は、墨線だけで兎や蛙に相撲をとらせ、絵だけで笑いと物語を生んだ。江戸に入ると鳥羽絵や草双紙が庶民の娯楽となり、1814年には葛飾北斎が素描集『北斎漫画』を刊行して「漫画」という語を広める。いずれもコマもフキダシも持たず近代マンガとの直接の血縁は薄いが、「絵を読む」文化の土壌はここで育った。",
    works: ["chojugiga", "hokusai"],
  },
  {
    era: "1900s—1930s",
    title: "近代マンガの誕生",
...
// マンガ史年表の本体（日英共通）。各時代の英訳は data/content.en.json の history
export default function HistoryBody({ lang = "ja" }: { lang?: Lang }) {
  return (
    <div className="page">
      <div className="page-en">HISTORY OF MANGA</div>
      <h1>{t("nav.history", lang)}</h1>
      <p className="page-lead">
        {t("hist.lead", lang)}
        <Link href={lp(lang, "/")} style={{ color: "#7fb3ff" }}>
          {t("nav.map", lang)}
        </Link>
        {t("hist.leadTail", lang)}
      </p>
      <div className="timeline">
        {ERAS.map((e) => (
          <div key={e.era} className="tl-item">
            <div className="era">{historyText(e.era, "era", e.era, lang)}</div>
            <h3>{historyText(e.era, "title", e.title, lang)}</h3>
            <p>{historyText(e.era, "body", e.body, lang)}</p>
            <div className="tl-works">
              ...
              {e.works.map((wid, i) => {
                ...
                return (
                  <span key={wid}>
                    {i > 0 && " / "}
                    <Link href={lp(lang, `/works/${wid}`)}>
                      {lang === "en" ? workTitle(w, lang) : `『${w.title}』`}
                    </Link>
                  </span>
                );
              })}
